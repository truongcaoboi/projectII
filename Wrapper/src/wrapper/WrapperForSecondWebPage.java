...
/**
 * webPage2 : https://mywork.com.vn/
 * 
 * 'div.el1.vb.vb-visible div ul li a'
 * 
...
public class WrapperForSecondWebPage {
    ...
    public Vector<Work> getWorks() {
        ...
        System.out.println("luong link: "+this.linkJobs.size());
        InputStream inStream = null;
        String link = "";
        Document doc = null;
        for (int i = 0; i < this.linkJobs.size(); i++) {
            try {
                ...
                inStream = new URL(link).openStream();
                doc = Jsoup.parse(inStream, "UTF-8", link);
                Element cover = doc.selectFirst("div.detail-content.container");
                String titleJob = cover.selectFirst("div div div header h1 span").text();
                Element company = cover.selectFirst("#left-content article div.offset10 h3 a");
